...
from distributional.model import GaussianPolicy
...
class CODAC(object):
    ...
    def get_single_tau(self):
        presum_tau = torch.rand(self.num_quantiles, device=self.device).detach() + 0.1
        presum_tau /= presum_tau.sum(dim=-1, keepdims=True)
        ...
        if self.risk_type == 'cvar':
            tau_hat *= self.risk_param

        return tau_hat

    def calc_critics(self, seq_state_list):
        # list of (seq_len, state_dim)
        seq_len_list = [len(s) for s in seq_state_list]
        max_len = max(seq_len_list)
        batch_len = len(seq_state_list)
        padded_tensor = torch.zeros((batch_len, max_len, self.state_dim), device=self.device)
        for i, seq in enumerate(seq_state_list):
            padded_tensor[i, :seq_len_list[i], :] = torch.tensor(seq, device=self.device)

        with torch.no_grad():
            tau = self.get_single_tau()
            tau_repeated = torch.repeat_interleave(tau.view(1, -1), batch_len*max_len, dim=0)
            sampled_a, _, _ = self.policy.sample(padded_tensor) # (B, max_len, dim(a))

            dist_z1 = self.zf1(padded_tensor.view(-1, self.state_dim), sampled_a.view(-1, self.action_dim), tau_repeated) # (B, max_len, 1)
            dist_z2 = self.zf2(padded_tensor.view(-1, self.state_dim), sampled_a.view(-1, self.action_dim), tau_repeated)
            
            v1 = torch.mean(dist_z1.view(batch_len, max_len, -1), dim=-1, keepdim=True)
            v2 = torch.mean(dist_z2.view(batch_len, max_len, -1), dim=-1, keepdim=True)
            v = torch.min(v1, v2).view(batch_len, max_len, 1)

            seq_ciritics = []
            for i, original_len in enumerate(seq_len_list):
                seq_ciritics.append(v[i][:original_len].cpu().detach().numpy())
